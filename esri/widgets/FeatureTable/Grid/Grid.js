...
//>>built
define("../../../chunks/_rollupPluginBabelHelpers ../../../chunks/tslib.es6 ../../../core/events ../../../core/HandleOwner ../../../core/maybe ../../../core/reactiveUtils ../../../core/accessorSupport/decorators/property ../../../core/accessorSupport/decorators/cast ../../../core/arrayUtils ../../../core/accessorSupport/decorators/subclass ../../Widget ./GridViewModel ../../support/widgetUtils ../../support/decorators/messageBundle ../../support/jsxFactory".split(" "),function(ib,y,Ja,ae,ka,Ka,z,
...
null;d.messages=null;d.viewModel=new ee;d.visibleElements={...$d};return d}ib._inheritsLoose(a,b);var c=a.prototype;c.initialize=function(){this.handles.add([this.columns.on("change",()=>this.onColumnChange()),Ka.watch(()=>this.viewModel?.size,()=>this._updateGridSize()),Ka.watch(()=>this.store?.state,(d,e)=>{"ready"!==d||"loaded"!==e&&"error"!==e||this.refreshPageCache()}),Ka.on(()=>this.highlightIds,"change",d=>{const {added:e,removed:f}=d;d=e.map(h=>this.store?.getLocalItemByKey(h)||{objectId:h});
const g=f.map(h=>this.store?.getLocalItemByKey(h)||{objectId:h});this._deselectRows(g);this._selectRows(d)}),Ka.on(()=>this._grid?.$?.table,"scroll",()=>this.viewModel?.closeColumnMenus())])};c.destroy=function(){this.handles.removeAll();this.resetColumns();this.columns?.destroy()};c.resetColumns=function(){this.columns.items.forEach(d=>d.destroy());this.columns.removeAll()};c.castVisibleElements=function(d){return{...$d,...d}};c.render=function(){const {columnMenus:d}=this.visibleElements;return Y.tsx("div",
...
this.scheduleRender()};c.notifyResize=function(){this._grid?.notifyResize()};c.recalculateColumnWidths=function(){this._grid?.recalculateColumnWidths()};c.reset=function(){var d=ib._asyncToGenerator(function*(){this._isDataLoaded=!1;this._clearSelection();yield this.store?.reset();this.scrollToTop()});return function(){return d.apply(this,arguments)}}();c.refreshPageCache=function(){this._grid?.clearCache()};c.selectRows=function(d){d=d?.filter(f=>!this.highlightIds.includes(f.objectId));const e=
d.map(f=>f.objectId);e.length&&(this.highlightIds.addMany(e),this._selectRows(d))};c.deselectRows=function(d){const e=d?.map(f=>f.objectId)||[];e.length&&(this.highlightIds.removeMany(e),this._deselectRows(d))};c.showColumn=function(d){this.viewModel?.showColumn(d);this.scheduleRender()};c.sort=function({path:d,direction:e}){this.viewModel?.sortColumn(d,e);this.notifyChange("sortOrders")};c.scrollToIndex=function(d){this._grid?.scrollToIndex(d)};c.scrollToTop=function(){this._grid?.scrollToIndex(0)};
c.isEditingActive=function(){return!!this._getActiveEditInfo().length};c.onColumnChange=function(){this._syncColumnRenderers()};c._afterGridCreate=function(d){const e=this._grid=d;e.setAttribute("theme","compact column-borders");customElements.whenDefined("vaadin-grid").then(()=>this._addGridEventListeners());e.__updateSorter=f=>{var g=e._sorters;const h=!g.includes(f);if(f.direction||!h)f._order=null,e.multiSort?(e._removeArrayItem(g,f),f.direction&&(ka.isSome(f._initialOrder)?(h?g[f._initialOrder]=
f:g.splice(f._initialOrder,0,f),f._initialOrder=null):g.unshift(f)),e.__updateSortOrders()):f.direction&&(g=g.filter(k=>k!==f),e._sorters=[f],g.forEach(k=>{k._order=null;k.direction=null}))}};c._afterGridUpdate=function(d){this._grid||(this._grid=d)};c._afterColumnCreate=function(d){this._applyRenderersToColumnElement(d);this._columnElements.push(d)};c._afterColumnRemoved=function(d){d=this._columnElements.indexOf(d,0);-1<d&&this._columnElements.splice(d,1)};c._syncColumnRenderers=function(){this._columnElements.forEach(d=>
{this._applyRenderersToColumnElement(d)})};c._updateGridSize=function(){this._grid&&(this._grid.size=this.size||0,this.scheduleRender())};c._getActiveEditInfo=function(){return!this.columns||this.columns.length?[]:this.columns.filter(d=>"activeEditInfo"in d).map(d=>d.activeEditInfo).items};c._addGridEventListeners=function(){const d=this._grid;ka.assertIsSome(d);this.handles.add([Ja.on(d,"click",e=>this._onRowClick(e)),Ja.on(d,"selected-items-changed",e=>this._onSelectionChange(e)),Ja.on(d,"sorter-changed",
()=>this.notifyChange("sortOrders")),Ja.on(d,"loading-changed",e=>{this._isDataLoaded||!1!==e.detail.value||(this._isDataLoaded=!0,this._selectRowsOnLoad())})])};c._onRowClick=function(d){var e=this._grid;ka.assertIsSome(e);e=e.getEventContext(d);const {item:f,section:g}=e;f&&g&&"details"!==g&&"header"!==g&&this.emit("row-click",{context:e,native:d})};c._clearSelection=function(){this._grid&&(this.highlightIds.removeAll(),this._grid.selectedItems=[],this._grid.render())};c._selectRowsOnLoad=function(){const d=
this._grid;if(d){var e=d.selectedItems.slice();(this.highlightIds?.filter(f=>0>e.findIndex(g=>g.objectId===f))).forEach(f=>d.selectedItems.push(this.store?.getLocalItemByKey(f)||{objectId:f}));d.render()}};c._selectRows=function(d){const e=this._grid;if(e){var f=e.selectedItems.slice();d.filter(g=>0>f.findIndex(h=>h.objectId===g.objectId)).forEach(g=>e.selectedItems.push(g));e.render()}};c._deselectRows=function(d){const e=this._grid;if(e){var f=e.selectedItems.slice();d.forEach(g=>{const h=f.findIndex(k=>
k.objectId===g.objectId);-1<h&&f.splice(h,1)});e.selectedItems=f;e.render()}};c._onSelectionChange=function(d){if("selectedItems.splices"===d.detail.path){const {removed:e,object:f}=d.detail.value.indexSplices[0];d=f.filter(h=>!this.highlightIds.includes(h.objectId)).map(h=>h.objectId);const g=e.filter(h=>this.highlightIds.includes(h.objectId)).map(h=>h.objectId);this.highlightIds.removeMany(g);this.highlightIds.addMany(d)}};c._applyRenderersToColumnElement=function(d){const e=this.viewModel.findColumn(d.path);
