...
a.fragment.include(t.BlendModes,c);a.fragment.code.add(d.glsl`
    vec4 getBackground(vec2 uv) {
      return ${g?d.glsl`baseOpacity *`:""} ${l?d.glsl`vec4(0.0, 0.0, 0.0, 0.0)`:k?d.glsl`vec4(backgroundColor, 1.0)`:e?d.glsl`vec4(gridColor(uv), 1.0)`:d.glsl`${u.texelFetch(c,"fboColor","gl_FragCoord.xy")}`};
    }
    ...
    vec4 blendLayers(vec4 bgColor, vec4 colorLayer, float opacity) {
      ${z?d.glsl`
          ...
          return ${!A&&(m&&!g||l)?d.glsl`colorLayer * opacity;`:d.glsl`bgColor * (1.0 - composeAlpha) + colorLayer * opacity;`}`}
    }`)};b.TileBackgroundPassParameters=y;Object.defineProperty(b,Symbol.toStringTag,{value:"Module"})});