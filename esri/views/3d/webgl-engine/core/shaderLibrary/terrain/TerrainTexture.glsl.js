  ...
  void forwardTextureCoordinatesWithTransform(in vec2 uv) {
    ...
    vec4 getColor(vec4 color, vec2 uv, vec3 opacities) {
      ${b||n?e.glsl`
              ...
              vec4 bg = vec4(${n?e.glsl`backgroundColor`:e.glsl`gridColor(uv)`} * opacities.y, opacities.y);
              vec4 layer = color * opacities.z;
              return (bg * (1.0 - layer.a) + layer) * opacities.x;`:e.glsl`return color;`}
...
}`)};f.Texture2DUniform=m;Object.defineProperty(f,Symbol.toStringTag,{value:"Module"})});