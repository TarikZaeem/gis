...
}`);switch(c.pbrMode){case d.PBRMode.Disabled:case d.PBRMode.WaterOnIntegratedMesh:case d.PBRMode.Water:e.include(h.MainLighting,c);a.code.add(b.glsl`vec3 evaluateSceneLighting(vec3 normalWorld, vec3 albedo, float shadow, float ssao, vec3 additionalLight)
{
...
inputs.averageAmbientRadiance = ambientLightIrradianceComponent[1] * (1.0 + GROUND_REFLECTANCE[1]);`);a.code.add(b.glsl`
        ...
        ${c.pbrMode===d.PBRMode.Schematic?b.glsl`vec3 outColor = pow(max(vec3(0.0), outColorLinear - 0.005 * inputs.averageAmbientRadiance), vec3(INV_GAMMA_SRGB));`:b.glsl`vec3 outColor = pow(blackLevelSoftCompression(outColorLinear, inputs), vec3(INV_GAMMA_SRGB));`}
        return outColor;
      }
    `);break;case d.PBRMode.Terrain:case d.PBRMode.TerrainWithWater:e.include(h.MainLighting,c);a.code.add(b.glsl`const float roughnessTerrain = 0.5;
