...
p.fromValues(1,1);return e}n._inheritsLoose(f,d);return f}(k.VisualVariablePassParameters);l.PathVertexPosition=function(d,f){const e=h.VertexAttribute.FEATUREVALUE;d.attributes.add(e,"vec4");const a=d.vertex;a.code.add(c.glsl`
  bool isCapVertex() {
    return ${e}.w == 1.0;
  }
  `);a.uniforms.add(new r.Float2PassUniform("size",b=>b.size));f.vvSize?(a.uniforms.add(new g.Float3PassUniform("vvSizeMinSize",b=>b.vvSizeMinSize)),a.uniforms.add(new g.Float3PassUniform("vvSizeMaxSize",b=>b.vvSizeMaxSize)),a.uniforms.add(new g.Float3PassUniform("vvSizeOffset",b=>b.vvSizeOffset)),a.uniforms.add(new g.Float3PassUniform("vvSizeFactor",b=>b.vvSizeFactor)),a.code.add(c.glsl`
    vec2 getSize() {
      return size * clamp(vvSizeOffset + ${e}.x * vvSizeFactor, vvSizeMinSize, vvSizeMaxSize).xz;
    }
    `)):a.code.add(c.glsl`vec2 getSize(){
return size;
}`);f.vvOpacity?(a.constants.add("vvOpacityNumber","int",8),a.uniforms.add([new m.FloatsPassUniform("vvOpacityValues",b=>b.vvOpacityValues,8),new m.FloatsPassUniform("vvOpacityOpacities",b=>b.vvOpacityOpacities,8)]),a.code.add(c.glsl`
...
return color;
}`);f.vvColor?(a.constants.add("vvColorNumber","int",k.vvColorNumber),a.uniforms.add([new m.FloatsPassUniform("vvColorValues",b=>b.vvColorValues,k.vvColorNumber),new t.Float4sPassUniform("vvColorColors",b=>b.vvColorColors,k.vvColorNumber)]),a.code.add(c.glsl`
    vec4 getColor() {
      float value = ${e}.y;
