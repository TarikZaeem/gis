...
color = mix(color, vec4(innerColor.rgb, innerAlpha), innerAA);`)),f.code.add(a.glsl`vec4 finalColor = blendStipple(color, stippleAlpha);`),b.falloffEnabled&&(f.uniforms.add(new l.FloatPassUniform("falloff",e=>e.falloff)),f.code.add(a.glsl`finalColor.a *= pow(max(0.0, 1.0 - abs(vLineDistanceNorm * gl_FragCoord.w)), falloff);`)));f.code.add(a.glsl`
    ...
    ${b.output===g.ShaderOutput.Alpha?a.glsl`gl_FragColor = vec4(finalColor.a);`:""}
    ${b.output===g.ShaderOutput.Color?a.glsl`gl_FragColor = highlightSlice(finalColor, vpos);`:""}
    ${b.output===g.ShaderOutput.Color&&b.transparencyPassType===Q.TransparencyPassType.Color?"gl_FragColor \x3d premultiplyAlpha(gl_FragColor);":""}
    ${b.output===g.ShaderOutput.Highlight?a.glsl`gl_FragColor = vec4(1.0);`:""}
