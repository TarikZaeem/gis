...
"vec4");d.attributes.add(q.VertexAttribute.SIZE,"vec2");d.attributes.add(q.VertexAttribute.AUXPOS2,"vec4");f.code.add(c.glsl`
    ...
    `);var m=c.glsl`vec2 uv01 = floor(uv0);
vec2 uv = uv0 - uv01;
...
posProj = alignToPixelOrigin(posProj, viewport.zw);
    ...
    ${a.output===k.ShaderOutput.ObjectAndLayerIdColor?c.glsl`vcolor.a = 1.0;`:""}

    bool alphaDiscard = vcolor.a < ${c.glsl.float(l.symbolAlphaCutoff)};
    ${e?`alphaDiscard = alphaDiscard && outlineColor.a < ${c.glsl.float(l.symbolAlphaCutoff)};`:""}
    if (alphaDiscard) {
  ...
  `);g.uniforms.add(new Q.Texture2DPassUniform("tex",b=>b.texture));m=a.debugDrawLabelBorder?c.glsl`(isBorder > 0.0 ? 0.0 : ${c.glsl.float(l.defaultMaskAlphaCutoff)})`:c.glsl.float(l.defaultMaskAlphaCutoff);e=c.glsl`
    ...
    ${e?c.glsl`
      ...
      if (outlineSize > 0.25) {
        ...
        if (
          ...
          fillPixelColor.a + outlinePixelColor.a < ${c.glsl.float(l.symbolAlphaCutoff)}
        ) {
          discard;
        }
  ...
  `;a.output===k.ShaderOutput.Alpha&&g.code.add(c.glsl`
      void main() {
        ${e}
        gl_FragColor = vec4(gl_FragColor.a);
      }
      `);a.output===k.ShaderOutput.ObjectAndLayerIdColor&&g.code.add(c.glsl`
      void main() {
        ${e}
        outputObjectAndLayerIdColor();
      }
      `);a.output===k.ShaderOutput.Color&&g.code.add(c.glsl`
    void main() {
      ${e}
      ${a.transparencyPassType===R.TransparencyPassType.FrontFace?"gl_FragColor.rgb /\x3d gl_FragColor.a;":""}
    }
    `);a.output===k.ShaderOutput.Highlight&&(d.include(J.OutputHighlight,a),g.code.add(c.glsl`
    void main() {
      ${e}
