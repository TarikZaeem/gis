...
vec4 finalColor = mix(uColor, shadedColor, shadingFactor);`)):f.code.add(d.glsl`vec4 finalColor = uColor;`);f.code.add(d.glsl`
      ...
      ${a.output===h.ShaderOutput.Alpha?d.glsl`gl_FragColor = vec4(finalColor.a);`:""}

      ${a.output===h.ShaderOutput.Color?d.glsl`gl_FragColor = highlightSlice(finalColor, vWorldPosition); ${a.transparencyPassType===E.TransparencyPassType.Color?"gl_FragColor \x3d premultiplyAlpha(gl_FragColor);":""}`:""}
    }
